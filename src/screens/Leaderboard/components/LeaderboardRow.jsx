...
export function LeaderboardRow({ score, isCurrentUser }) {
  ...
  const linkClassNames = tail`
        ...
        ${(props) =>
          props.type === "user" ? "text-on-primary" : "text-primary"}
    `;

  return (
    <LeaderboardRowWrapper position={score.position} type={type}>
      <td className="text-sm font-bold">{score.position}</td>
      <td>{score.user.username}</td>
      <td>{score.points}</td>
      <td className="flex flex-col items-center justify-center">
        <span className="inline text-sm">{performancePercentual}</span>
        <meter
          low={0.35}
          high={0.7}
          optimum={1}
